...
export const signup = async (fullname, email, phone, password) => {
  try {
    const response = await axios.post(`${API_URL}/api/vendorapp/auth/signup`, {
      fullname,
      email,
      phone,
      password,
...
export const signin = async (email, password) => {
  try {
    const response = await axios.post(`${API_URL}/api/vendorapp/auth/signin`, {
      email,
      password,
    });
    return response.data;
...
export const signinwithgoogle = async (data) => {
  try {
    const response = await axios.post(
      `${API_URL}/api/vendorapp/auth/signinwithgoogle`,
      {
        email: data.email,
        fullname: data.name,
